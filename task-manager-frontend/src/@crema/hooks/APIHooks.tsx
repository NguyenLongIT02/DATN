...
export const useGetDataApi = <T,>(
      ...
      jwtAxios
        ...
        .catch((error: any) => {
          ...
          if (error.response?.data?.message) {
            ...
            fetchError(error.message || "An error occurred");
            if (callbackFun) callbackFun(error);
          }
          setLoading(false);
        });
    };
    if (allowApiCall && !resStateRef.current && initialUrl) fetchData();
    return () => {
      didCancelRef.current = true;
    };
  }, [initialUrl, queryParams, allowApiCall]);
  const setDataCallback = useCallback((data: T) => setData(data), []);
  const setLoadingCallback = useCallback((loading: boolean) => setLoading(loading), []);
  const updateInitialUrlCallback = useCallback((value: string) => updateInitialUrl(value), []);
  const setQueryParamsCallback = useCallback((queryParams: object) => setQueryParams(queryParams), []);
  const reCallAPICallback = useCallback(() => reCallAPI(), [queryParams]);
  ...
  return [
    ...
    },
    {
      ...
      updateInitialUrl: updateInitialUrlCallback,
      setQueryParams: setQueryParamsCallback,
      reCallAPI: reCallAPICallback,
    },
  ] as const;
};
...
const handleApiResponse = <T,>(
  ...
  if (isRequestSuccessful(data.status)) {
    // Unwrap SuccessResponse<T> format: response.data.data
    const actualData =
      (data.data as any)?.data !== undefined
