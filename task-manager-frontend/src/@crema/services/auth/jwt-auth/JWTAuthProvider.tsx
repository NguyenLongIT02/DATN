  ...
  useEffect(() => {
    const getAuthUser = async () => {
      ...
      try {
        ...
        try {
          const { data } = await jwtAxios.get("/auth/me");
          if (data && data.data) {
            setJWTAuthData({
              user: data.data,
              isLoading: false,
          ...
          console.error("Failed to fetch user profile, but token is valid:", profileError);
          setJWTAuthData({
            user: undefined,
            isLoading: false,
            isAuthenticated: true,
  ...
  const signInUser = async ({
    ...
    try {
      ...
      if (data.data && data.data.access_token) {
        ...
        // Extract user data from the dedicated 'user' field in TokenResponse
