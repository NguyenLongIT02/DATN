...
export const exportMembersToPdf = async (
  ...
  const pmCount     = members.filter((m) => String(m.role) === "PM" || String(m.role) === "Project Manager").length;
  const tlCount     = members.filter((m) => String(m.role) === "TEAM_LEAD" || String(m.role) === "Team Lead").length;
  const memberCount = members.filter((m) => String(m.role) === "MEMBER" || String(m.role) === "Member").length;
