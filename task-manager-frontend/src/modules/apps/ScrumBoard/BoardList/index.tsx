...
const BoardList = () => {
  ...
  const onAddBoard = (name: string, startDate?: string, endDate?: string) => {
    if (selectedBoard) {
      ...
      putDataApi("/scrumboard/board/edit/board", infoViewActionsContext, {
        ...
        .catch((error) => {
          showOperationErrorNotification("cập nhật dự án", error.message);
        });
    } else {
      postDataApi<BoardObjType>(
        ...
        .catch((error) => {
          showOperationErrorNotification("tạo dự án", error.message);
        });
    }
  };
  ...
  const onDeleteButtonClick = (board: BoardObjType) => {
    Modal.confirm({
      title: "Xóa dự án",
      content: `Bạn có chắc chắn muốn xóa dự án "${board.name}"? Hành động này không thể hoàn tác.`,
      okText: "Xóa",
      okType: "danger",
      cancelText: "Hủy",
      onOk: () => {
        ...
        deleteDataApi(
          ...
          .catch((error) => {
            showOperationErrorNotification("xóa dự án", error.message);
          });
      },
    });
  };
