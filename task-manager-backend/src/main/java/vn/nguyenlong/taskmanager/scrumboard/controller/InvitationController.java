...
public class InvitationController {
    ...
    @Operation(summary = "Get pending invitations", description = "Get all pending board invitations for the current user")
    public SuccessResponse<java.util.List<Map<String, Object>>> getPendingInvitations(Principal principal) {
        Long currentUserId = extractUserIdFromPrincipal(principal);
        return ResponseUtil.ok(HttpStatus.OK.value(), "Success", invitationService.getPendingInvitations(currentUserId));
    }

    @PostMapping("/invitations/decline")
    @Operation(summary = "Decline invitation", description = "Decline a board invitation using the token")
    public SuccessResponse<String> decline(@RequestBody Map<String, String> body) {
        String token = body.get("token");
        invitationService.decline(token);
        return ResponseUtil.ok(HttpStatus.OK.value(), "Invitation declined successfully", "Declined");
    }
